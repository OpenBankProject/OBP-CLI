...
def importAccounts(spreadsheet=None, sheet_name=None):
  ...
  for index, account in enumerate(sheetdata[1:][0][1:]): #skips sheetname, and header
    ...
    try:
      ...
      account_id = get_value(2, account)
  ...
  for payload in accounts:
      ...
      url = get_config('OBP_API_HOST') + '/obp/v3.1.0/banks/{BANK_ID}/accounts/{ACCOUNT_ID}'.format(BANK_ID=bank_id, ACCOUNT_ID=account_id)
